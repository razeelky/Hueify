...
export default function ColorHistory() {
  ...
  const fetchColorData = async (colors: string[]) => {
    try {
      ...
      setError(error instanceof Error ? error.message : "Failed to load color data");
      setIsLoading(false);
    }
  };
  ...
  const paginateColors = () => {
    ...
    return colorData.slice(startIndex, startIndex + itemsPerPage);
  };
    ...
    <Layout>
      {isLoading ? (
        ...
        <div className="container mx-auto p-4 min-h-[80vh]">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {paginateColors().map((color, index) => (
              <Card key={index} className="overflow-hidden">
                <CardContent className="p-0">
                  <div
                    className="h-32 w-full"
