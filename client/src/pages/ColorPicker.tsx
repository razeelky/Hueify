...
export default function ColorPaletteApp() {
  ...
  // Updated PDF configuration with better margins and styling
...
>>>>>>> c6cae2b (Save current Hueify updates)
  ...
  const fetchColorData = async (input: string) => {
    try {
      ...
      setError(
        error instanceof Error ? error.message : "An error occurred while fetching color data."
      );
      setColorData(null);
    } finally {
      setLoadingColorData(false);
  ...
  const fetchColorInspirations = async () => {
    try {
      ...
      const getRandomColors = (colors: ColorInspiration[], count: number) => {
      ...
      setError(
        error instanceof Error
          ? error.message
          : "An error occurred while fetching color inspirations."
      );
    } finally {
      setLoadingInspiration(false);
    }
            ...
            <CardContent>
              <div className="space-y-4">
                ...
                {loadingColorData ? (
                  ...
                  <div className="size-full">
                    {/* Updated PDF target with better styling */}
                    <div 
                      ref={targetRef} 
                      className="space-y-2 font-mono bg-white p-4 rounded-lg"
                      style={{ color: 'black' }}
                    >
                      <div className="flex items-center space-x-2">
                        <div
                    ...
                    <div className="flex gap-3 mt-4">
                      <Button className="font-sans" onClick={() => {setColorData(null); setColorInput("");}}>
                        Clear
                      </Button>
              ...
              {selectedImage && (
                <div className="relative">
                  ...
                  <img
                    ...
                    onClick={extractColorFromImage}
                  />
                  <Button className="mt-5" onClick={() => setSelectedImage(null)}>
                    Clear Image
                  </Button>
                </div>
              )}
